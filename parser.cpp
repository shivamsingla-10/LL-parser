...
int main(int argc, char const *argv[])
{
	...
	if(!LL_grammar){
		cout<<endl<<"Grammar is not LL(1)"<<endl;
		return 0;
	...
	stack_content.push_back(start_sym);
	...
	cout<<endl<<endl<<endl;
	cout<<left<<setw(40)<<"stack"<<left<<setw(40)<<"input string"<<endl<<endl;
	bool accepted = true;
	while(!stack_content.empty() && !input_string.empty()) {
		// If stack top same as input string char remove it
		cout<<left<<setw(40)<<stack_content<<left<<setw(40)<<input_string<<endl;

		if(input_string[0] == stack_content.back()) {
			stack_content.pop_back();
			input_string.erase(0, 1);
		}
		else if(!isupper(stack_content.back())) {
			...
			char stack_top = stack_content.back();
